...
/*   Updated: 2025/02/13 15:45:13 by edelplan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	wall_hit(t_data *data, double x, double y)
{
		...
		&& map_x < (int)ft_strlen(data->mapdata->map[map_y]))
	{
		if (data->mapdata->map[map_y][map_x] == '1')
			return (1);
		if (data->mapdata->map[map_y][map_x] == 'D')
		{
			if (data->mapdata->door_states[map_y][map_x] == OPEN)
				return (0);
			return (2);
		}
	}
...
int	door_hit(t_data *data, double x, double y)
{
	...
	if (data->mapdata->map[map_y]
		&& map_x < (int)ft_strlen(data->mapdata->map[map_y]))
	{
		return (data->mapdata->map[map_y][map_x] == 'D');
	}
	return (0);
}
