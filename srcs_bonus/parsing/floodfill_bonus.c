...
/*   Updated: 2025/02/17 15:07:26 by mchouikr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_zero_neighbors(char **map_array, int x, int y)
{
	...
	if (!is_valid_pos(map_array, x, y - 1) || ft_isspace(map_array[y][x + 1]
		|| !is_valid_pos(map_array, x + 1, y)
		|| ft_isspace(map_array[y][x - 1])
		|| !is_valid_pos(map_array, x, y + 1)
		|| ft_isspace(map_array[y + 1][x])
		|| !is_valid_pos(map_array, x - 1, y)
		|| ft_isspace(map_array[y - 1][x])))
		return (false);
	return (true);
...
bool	flood_fill_map(char **map_array)
{
	...
	while (map_array[y])
	{
		...
		while (map_array[y][x])
		{
			if (map_array[y][x] == '0' || map_array[y][x] == 'D'
				|| map_array[y][x] == 'C')
...
int	check_if_closed(char *oneline_map, t_mapdata *mapdata)
{
	char	**map;
	int		y;
	bool	is_valid;
	...
	map = create_map_array(oneline_map, mapdata);
	y = -1;
	is_valid = true;
	while (map[++y] && is_valid)
	{
		x = -1;
		while (map[y][++x] && is_valid)
		{
	...
	if (!is_valid)
		return (ft_fprintf(2, MAP_NOT_CLOSED), free_array(map), FAILURE);
	return (free_array(map), SUCCESS);
}
